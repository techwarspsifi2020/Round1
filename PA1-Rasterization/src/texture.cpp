...
Color Texture::sample_bilinear(Vector2D uv) {
  ...
  // Hint: look at sample_nearest
    ...
    Color c_bl = Color(&(mipmap[level].texels[4*(yd * w + xl)]));
    Color c_br = Color(&(mipmap[level].texels[4*(yd * w + xr)]));
    Color c_tl = Color(&(mipmap[level].texels[4*(yu * w + xl)]));
    Color c_tr = Color(&(mipmap[level].texels[4*(yu * w + xr)]));

    Color c_iu = abs((xr-upix)/(xr - xl))*c_tl + abs((xl - upix)/(xr - xl)) * c_tr;
    Color c_id = abs((xr-upix)/(xr - xl))*c_bl + abs((xl - upix)/(xr - xl)) * c_br;

    Color c_if = abs((yd-vpix)/(yu - yd)) * c_iu + abs((yu-vpix)/(yu - yd)) * c_id;
