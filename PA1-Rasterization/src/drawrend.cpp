...
bool inside_tri(float Ax, float Ay,
            float Bx, float By,
            float Cx, float Cy,
            float Px, float Py) {
...
bool corners_in(int x0, int y0, int x1, int y1, int x2, int y2, int x, int y, int o_factor)
{
    return  (int)inside_tri(x0, y0, x1, y1, x2, y2, x, y) + 
            (int)inside_tri(x0, y0, x1, y1, x2, y2, x + o_factor, y) + 
            (int)inside_tri(x0, y0, x1, y1, x2, y2, x, y + o_factor) + 
            (int)inside_tri(x0, y0, x1, y1, x2, y2, x + o_factor, y + o_factor);
}
...
// --------------PART 1 CODE----------------------------------------
    ...
    for(int x = round(min_x-0.5f); x < round(max_x+0.5f); x += o_factor)
    {
      for(int y = round(min_y-0.5f); y < round(max_y+0.5f); y += o_factor)
      {

        //optimization------------------
        int corners = corners_in(x0, y0, x1, y1, x2, y2, x + off, y + off, o_factor);

        if(corners == 4)
        {
          for(int i = x; i < x + o_factor; i++)
            for(int j = y; j < y + o_factor; j++)
          ...
          for(int i = x; i < x + o_factor; i++)
            {
              for(int j = y; j < y + o_factor; j++)
              {
                if(inside_tri(x0, y0, x1, y1, x2, y2, i, j))
...
/*
 // --------------PART 2 CODE----------------------------------------
    ...
    float min_x = min(min(x0, x1), x2);
    float min_y = min(min(y0, y1), y2);
    int sps = samplebuffer[0][0].samples_per_side;
    float off = (1.0f/sps);
    float factor = 1.0f;


    for(int x = round(min_x-0.5f); x < round(max_x+0.5f); x++)
    {
        for(int y = round(min_y-0.5f); y < round(max_y+0.5f); y++)
        {
            for(int x_subp = 0; x_subp < sps; x_subp++)
            {
              for(int y_subp = 0; y_subp < sps; y_subp++)
              {
                if(inside_tri(x0, y0, x1, y1, x2, y2, x + off*(x_subp) + 0.5f*off, y + off*(y_subp) + 0.5f*off ))
                {
                    samplebuffer[y][x].fill_color(x_subp, y_subp, color);
                }
              }
            }
        }
...
//--------------PART 4 CODE-------------------------------------------
    ...
    for(int x = round(min_x-0.5f); x < round(max_x+0.5f); x++)
    {
        for(int y = round(min_y-0.5f); y < round(max_y+0.5f); y++)
        {
            if(inside_tri(x0, y0, x1, y1, x2, y2, x + off, y + off))
            {
                if(!tri)
                    samplebuffer[y][x].fill_pixel(color);
                else
                    samplebuffer[y][x].fill_pixel(tri->color(x0, y0, x1-x0, y1-y0, x2-x0, y2-y0, x-x0, y-y0));
            }
        }
    }
...
//                     sp.psm = psm;
//                     samplebuffer[y][x].fill_pixel(tri->color(x0, y0, x1-x0, y1-y0, x2-x0, y2-y0, x-x0, y-y0, sp));
//                 }
//             }
//         }
//     }
...
//     //--------------PART 5 CODE END-------------------------------------------
   ...
   // --------------PART 6 CODE -------------------------------------------
    ...
    for( int i=0; i<3; i++)
    {
      ...
      vert[i].invSlope = (x[i+1]-x[i])/(1.0f*y[i+1]-y[i]);
    }
...
void printET(std::vector<Coord>& t)
{
    cout << "ymin\tymax\txmin\t1/m" << endl;
    for (int i = 0; i < t.size(); i++)
    {
        cout << t[i].yMin << "\t" << t[i].yMax << "\t" << t[i].xMin << "\t" << t[i].invSlope << endl;
    }
}

void printAET(std::vector<Coord>& t)
{
    cout << "y\tymax\tx\t1/m" << endl;
    for (int i = 0; i < t.size(); i++)
    {
        cout << t[i].y << "\t" << t[i].yMax << "\t" << t[i].x << "\t" << t[i].invSlope << endl;
    }
}
...
void DrawRend::scanLine(Coord *verticess, int nVertices, Color c){
  ...
  for (int y = ymin; y<ymax; ++y)
  {
    ...
    if(AET.size() > 1)
    {
        if(AET.size() % 2 == 1)
        {
            for(int itr = 0; itr < AET.size()-1; itr++)
            {
                if(AET[itr].yMin == AET[itr+1].yMax)
                    AET.erase(AET.begin() + itr + 1);
                else if(AET[itr+1].yMin == AET[itr].yMax)
                    AET.erase(AET.begin() + itr);
                
            }
        }
        for (j = 1; j < AET.size(); j=j+2)
        {
            rasterize_line(ceil(AET[j-1].x),AET[j-1].y,floor(AET[j].x), AET[j].y,c);
        }
      ...
      while (j >= 0)
      {
          ...
          AET[j].x = AET[j].x + AET[j].invSlope;
