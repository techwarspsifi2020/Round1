 ...
 */
 Color ColorTri::color(float x0, float y0,float x1, float y1,float x2, float y2, float px, float py, SampleParams sp) {
   ...
   // Some variables that you might find helpful:
    ...
    double tA = 0.5 * (Matrix3x3(x1, y1, 0, x2, y2, 0, 0, 0, 1).det());
    double aA = 0.5 * (Matrix3x3(x1, y1, 0, px, py, 0, 0, 0, 1).det());
    double bA = 0.5 * (Matrix3x3(px, py, 0, x2, y2, 0, 0, 0, 1).det());
    double cA = 0.5 * (Matrix3x3(x1-px, y1-py, 0, x2-px, y2-py, 0, 0, 0, 1).det());

    double _alpha = cA/tA;
    double _beta = bA/tA;
    double _gamma = aA/tA;

    p_bary = Vector3D(_alpha, _beta, _gamma);

    return p0_col*p_bary.x + p1_col*p_bary.y + p2_col*p_bary.z;
 }
 ...
 Color TexTri::color(float x0, float y0,float x1, float y1,float x2, float y2, float px, float py, SampleParams sp) {
   ...
   // Some variables that you might find helpful:
    ...
    sp.p_uv = _alpha*p0_uv + _beta*p1_uv + _gamma*p2_uv; 
